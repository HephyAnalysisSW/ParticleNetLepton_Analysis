from    VH.Tools.helpers import mZ, getVarValue, getObjDict, deltaR
...
lhepartVars = ['eta', 'pt', 'phi', 'mass', 'pdgId', 'status']

def getLHEParts(c, lhepartVars=lhepartVars, lhepartColl="LHEPart"):
	return [getObjDict(c, lhepartColl+'_', lhepartVars, i) for i in range(int(getVarValue(c, 'n'+lhepartColl)))]

def getGENParts(c, genpartVars=lhepartVars, genpartColl="GenPart"):
	return [getObjDict(c, genpartColl+'_', genpartVars, i) for i in range(int(getVarValue(c, 'n'+genpartColl)))]

jetVars = ['eta','pt','phi', 'mass', 'btagDeepB', 'btagDeepFlavB', 'btagCSVV2', 'jetId', 'area', 'rawFactor', 'corr_JER']
...
def isAnalysisJet(j, ptCut=30, absEtaCut=2.4, puptCut=50, puIdWP=7, ptVar='pt', idVar='jetId', puid='puId', corrFactor=None):
  j_pt = j[ptVar] if not corrFactor else j[ptVar]*j[corrFactor]
  pu_pass = False if j_pt < puptCut and j[puid]<puIdWP else True
  return j_pt>ptCut and abs(j['eta'])<absEtaCut and pu_pass and ( j[idVar] > 1 if idVar is not None else True )
...
fatjetVars = ['pt', 'rawFactor', 'eta', 'phi', 'jetId', 'area', 'deepTagMD_HbbvsQCD', 'deepTagMD_TvsQCD', 'deepTagMD_WvsQCD', 'deepTagMD_ZHbbvsQCD', 'deepTagMD_ZHccvsQCD', 'deepTagMD_ZbbvsQCD', 'deepTagMD_ZvsQCD', 'deepTagMD_bbvsLight', 'deepTagMD_ccvsLight', 'deepTag_H', 'deepTag_QCD', 'deepTag_TvsQCD', 'deepTag_WvsQCD', 'deepTag_ZvsQCD', 'mass', 'msoftdrop', 'tau1', 'tau2', 'tau3', 'subJetIdx1', 'subJetIdx2']

def getFatJets(c, fatjetVars=fatjetVars, fatjetColl="FatJet"):
    return [getObjDict(c, fatjetColl+'_', fatjetVars, i) for i in range(int(getVarValue(c, 'n'+fatjetColl)))]

subjetVars = ['pt', 'rawFactor', 'eta', 'phi', 'mass', 'btagDeepB']

def getSubJets(c, subjetVars=subjetVars, subjetColl="SubJet"):
    return [getObjDict(c, subjetColl+'_', subjetVars, i) for i in range(int(getVarValue(c, 'n'+subjetColl)))]

def getCleanJets(jets,leptons,deltaR_cut=0.4):
	goodjet = [ True for j in jets ]
	for ij,j in enumerate(jets):
		for il, l in enumerate(leptons):
			if (deltaR(j,l)<deltaR_cut):
				goodjet[ij] = False
				break
	return ([ j for (ij,j) in enumerate(jets) if goodjet[ij] == True ])


genjetVars = ['pt','eta','phi','mass']

def getGenJets(c, genjetVars=genjetVars, genjetColl="GenJet"):
	return [getObjDict(c, genjetColl+'_', genjetVars, i) for i in range(int(getVarValue(c, 'n'+genjetColl)))]
...
def getEFTweights(c, collVars=['weight']):
    return [getObjDict(c, 'LHEReweightingWeight_', collVars, i) for i in range(int(getVarValue(c, 'nLHEReweightingWeight')))]

electronVars_data = ['pt','eta','phi', 'mass', 'pdgId','cutBased','miniPFRelIso_all','pfRelIso03_all','sip3d','lostHits','convVeto','dxy','dz','charge','deltaEtaSC', 'mvaFall17V2Iso_WP80', 'jetRelIso', 'mvaFall17V2Iso_WP90', 'vidNestedWPBitmap','mvaTOP', 'jetRelIso', 'jetIdx', 'sieie', 'hoe', 'eInvMinusPInv', 'pfRelIso04_all', 'mvaFall17V2noIso_WP80', 'mvaFall17V2noIso_WP90', 'lostHits']
electronVars = electronVars_data + []

muonVars_data = ['pt','eta','phi','mass','charge','pdgId', 'dxy', 'dz', 'looseId', 'mediumId', 'tightId', 'mvaId', 'mvaTOP', 'mvaTTH', 'miniPFRelIso_all', 'pfRelIso04_all', 'pfRelIso03_all', 'pfIsoId', 'jetRelIso', 'sip3d', 'jetIdx']
